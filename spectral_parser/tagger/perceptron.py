...
"""
Modifications made to the original file from NLTK's GitHub
"""
import random
...
class PerceptronTagger(TaggerI):
    ...
    def train(self, sentences, dev, save_loc=None, nr_iter=10):
        ...
        for iter_ in range(nr_iter):
            c = 0
            n = 0
            for sentence in tqdm(self._sentences, desc='Iter %d/%d' % (iter_ + 1, nr_iter)):
            ...
            print("Iter {:d}: Train={:.2f} Dev={:.2f}".format(iter_, _pc(c, n), self.evaluate(dev)*100))
    ...
    def _get_features(self, i, word, context, prev, prev2):
        ...
        add("bias")
        add("lower cased word", word.lower())
        # add("i suffix", word[-3:])
        add("i suffix", self._get_suffix(word[-3:]))
        ...
        add("i-1 suffix", self._get_suffix(context[i - 1][-3:]))
        ...
        if context[i] == "!HYPHEN":
            idx = word.find('-')
            add("left", word[:idx])
            add("right", word[idx+1:])
            add("left suffix", self._get_suffix(word[:idx]))
        return features
...
def _get_pretrain_model(train, dev):
    # trainig/dev data: [(word, tag)]
    # Train and test on English part of ConLL data (WSJ part of Penn Treebank)
    # Train: section 2-21
    # Test : section 22
    tagger = PerceptronTagger(load=False)
    print("Size of training and testing (sentence)", len(train), len(dev))
    # Train and save the model
    tagger.train(train, PICKLE)
    print("Accuracy : ", tagger.evaluate(dev))
